      ...
      <div className="min-h-screen bg-gray-50 p-2 sm:p-4">
        ...
        {isUpdatingStatus && !showOtpModal && (
          <div className="fixed inset-0 bg-black/30 backdrop-blur-sm flex items-center justify-center z-50">
            <div className="bg-white rounded-lg p-4 sm:p-6 shadow-xl flex items-center space-x-3 mx-4">
        ...
        <div className="space-y-4 sm:space-y-6">
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-3 sm:p-4 lg:p-6">
            <div className="flex items-start sm:items-center space-x-3 sm:space-x-4">
              <button
                ...
                className="p-1.5 sm:p-2 hover:bg-gray-100 rounded-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed shrink-0 mt-1 sm:mt-0"
              >
              ...
              <div className="min-w-0 flex-1">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-3 mt-1">
                  <span className={`inline-flex items-center px-2 sm:px-2.5 py-0.5 rounded-full text-xs font-medium ${statusConfig.color} ${isUpdatingStatus ? 'animate-pulse' : ''} w-fit`}>
                    {isUpdatingStatus ? (
            ...
            {/* Progress Button - Mobile responsive */}
            {showProgressButton && nextStatusLabel && (
              <div className="mt-4 sm:mt-6 flex justify-start">
                <button
                  onClick={() => handleStatusProgress(nextStatus)}
                  disabled={isUpdatingStatus}
                  className={`px-3 sm:px-4 py-2 rounded-lg font-medium transition-colors flex items-center space-x-2 text-sm sm:text-base ${nextStatus === 'Accepted'
            ...
            {updateProgressMutation.isError && !showOtpModal && (
              <div className="mt-4 bg-red-50 border border-red-200 rounded-lg p-3">
                <div className="flex items-start space-x-2 text-red-800">
                  ...
                  <span className="text-xs sm:text-sm break-words">{updateProgressMutation.error.message}</span>
                </div>
              </div>
            )}
            ...
            {updateProgressMutation.isSuccess && !isUpdatingStatus && (
              <div className="mt-4 bg-green-50 border border-green-200 rounded-lg p-3">
                <div className="flex items-center space-x-2 text-green-800">
                  ...
                  <span className="text-xs sm:text-sm">Order status updated successfully!</span>
                </div>
              </div>
            )}
          </div>
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-4 sm:p-6">
              ...
              <div className="space-y-3 sm:space-y-4">
                <div>
                  ...
                  <p className="text-sm sm:text-base text-gray-900 font-medium font-mono break-all">{currentOrder._id}</p>
                </div>
                <div>
                  <label className="text-xs sm:text-sm font-medium text-gray-600">Main Order ID</label>
                  <p className="text-sm sm:text-base text-gray-900 font-mono break-all">{currentOrder.mainOrderId}</p>
                </div>
                <div>
                  <label className="text-xs sm:text-sm font-medium text-gray-600">Order ID</label>
                  <p className="text-sm sm:text-base text-gray-900 font-mono break-all">{currentOrder.orderId}</p>
                </div>
                <div>
                  <label className="text-xs sm:text-sm font-medium text-gray-600">Vendor ID</label>
                  <p className="text-sm sm:text-base text-gray-900 font-mono break-all">{currentOrder.vendorId}</p>
                </div>
                <div>
                  <label className="text-xs sm:text-sm font-medium text-gray-600">Order Type</label>
                  <p className="text-sm sm:text-base text-gray-900 capitalize">{currentOrder.orderType || 'N/A'}</p>
                </div>
                <div className="text-xs sm:text-sm text-gray-600 space-y-1 pt-2">
                  <p><strong>Created:</strong> <span className="break-words">{formatDate(currentOrder.createdAt)}</span></p>
                  <p><strong>Last Updated:</strong> <span className="break-words">{formatDate(currentOrder.updatedAt)}</span></p>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-4 sm:p-6">
              ...
              <div className="space-y-3 sm:space-y-4">
                <div>
                  ...
                  <p className="text-sm sm:text-base text-gray-900 break-words">{currentOrder.companyName}</p>
                </div>
                <div>
                  <label className="text-xs sm:text-sm font-medium text-gray-600">Contact Person</label>
                  <p className="text-sm sm:text-base text-gray-900 break-words">{currentOrder.contactPerson}</p>
                </div>
                <div>
                  <label className="text-xs sm:text-sm font-medium text-gray-600">{isServiceOrder ? 'Service Address' : 'Delivery Address'}</label>
                  <p className="text-sm sm:text-base text-gray-900 leading-relaxed break-words">{currentOrder.deliveryAddress}</p>
                </div>
              </div>
            </div>
          </div>
          ...
          {showOtpModal && (
            <div className="fixed inset-0 bg-black/50 backdrop-blur-sm transition-opacity duration-300 flex items-center justify-center p-4 z-50">
              <div className="bg-white rounded-lg max-w-md w-full mx-4 p-4 sm:p-6 relative">
                ...
                <div className="flex items-center space-x-2 mb-3 sm:mb-4">
                  {modalContent.icon}
                  <h3 className="text-base sm:text-lg font-semibold text-gray-900">{modalContent.title}</h3>
                </div>
                ...
                <div className="mb-4">
                  ...
                  <input
                    ...
                    placeholder="12345"
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-green-500 text-center text-base sm:text-lg tracking-widest"
                    maxLength={5}
                    disabled={isUpdatingStatus}
                  />
                  {otpError && (
                    <div className="flex items-start space-x-1 mt-2 text-red-600 text-xs sm:text-sm">
                ...
                <div className="flex flex-col sm:flex-row space-y-2 sm:space-y-0 sm:space-x-3">
                  <button
                    ...
                    disabled={isUpdatingStatus}
                    className="flex-1 px-4 py-2 border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50 transition-colors disabled:opacity-50 disabled:cursor-not-allowed text-sm sm:text-base"
                  >
                    Cancel
                  </button>
                  <button
                    ...
                    className={`flex-1 px-4 py-2 text-white rounded-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center space-x-2 text-sm sm:text-base ${modalContent.buttonColor}`}
                  >
                    {isUpdatingStatus ? (
                      <>
                        <Loader2 className="h-4 w-4 animate-spin" />
                      ...
                      <span className="break-words">{modalContent.buttonText}</span>
                    )}
                  </button>
                </div>
              </div>
